...
if [[ "$backend" == "megatron" ]]; then
  ...
  configs=${config_dir}/llama2/vllm_rlhf_new.yaml
fi
...
if [[ "$model_size" == "llama2-7B" ]]; then
    [ -z "$policy_tp" ] && export policy_tp=2
    [ -z "$policy_pp" ] && export policy_pp=1
    [ -z "$ppo_policy_tp" ] && export ppo_policy_tp=2
    [ -z "$ppo_policy_pp" ] && export ppo_policy_pp=1
    [ -z "$reward_tp" ] && export reward_tp=2
    ...
    export num_gpu_ref=2
    export num_gpu_value=2
    export num_gpu_ppo_policy=2
    export num_gpu_ppo_value=2
